    ...
    <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
      {statsCards.map((stat, index) => (
        <motion.article
          ...
          whileHover={{ y: -6 }}
          className="group relative overflow-hidden rounded-2xl border border-black/10 bg-white p-6 shadow-sm transition duration-300 hover:shadow-[0_24px_45px_-28px_rgba(15,23,42,0.45)]"
        >
          ...
          <p className="mt-2 text-sm font-medium text-emerald-700">{stat.delta} this month</p>
        </motion.article>
      ))}
    </section>
  );
